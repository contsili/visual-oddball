...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\kt\\Git\\visual-oddball\\auditory_oddball_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[sound_1, trialnumber, crosshair],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        sound_1.setSound(soundFile, secs=0.2, hamming=True)
        ...
        while continueRoutine:
            ...
            if sound_1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sound_1.tStartRefresh + 0.2-frameTolerance or sound_1.isFinished:
                    # keep track of stop time/frame for later
            ...
            if trialnumber.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trialnumber.tStartRefresh + isi + 0.2-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if crosshair.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > crosshair.tStartRefresh + isi + 0.2-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        # Check if we've completed 100 trials (a block)
