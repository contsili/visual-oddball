...
# LSL (Lab Streaming Layer) for real-time data streaming
...
expName = 'visual_oddball'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\kt\\Git\\visual-oddball\\visual_oddball_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text='Experiment starting in 2 seconds',
        font='Arial',
    ...
    visual_stimulus = visual.ImageStim(
        win=win,
        name='visual_stimulus', units='height', 
    ...
    trialnumber = visual.TextStim(win=win, name='trialnumber',
        text='',
        font='Arial',
        pos=(-0.60, 0.45), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-2.0);
    ...
    text_norm_4 = visual.TextStim(win=win, name='text_norm_4',
        text='Have some rest, blink etc. then press SPACE to continue.',
        font='Arial',
        units='norm', pos=(0, 0), draggable=False, height=0.1, wrapWidth=1.8, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[visual_stimulus, trialnumber, crosshair],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        visual_stimulus.setImage(imageFile)
        # Run 'Begin Routine' code from code_2
        # Increment counter at the beginning of each trial
        trialnr = trials.thisN
        
        stimulus_pulse_started = False
        stimulus_pulse_ended = False
        
        trialnumber.setText("%d" % trialnr)
        ...
        while continueRoutine:
            ...
            if visual_stimulus.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if visual_stimulus.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > visual_stimulus.tStartRefresh + stimDuration-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # Hardware triggers for TMSI APEX EEG
            ...
            # if trialnumber is starting this frame...
            if trialnumber.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # update status
                trialnumber.status = STARTED
                trialnumber.setAutoDraw(True)
            
            # if trialnumber is active this frame...
            if trialnumber.status == STARTED:
                # update params
                pass
            
            # if trialnumber is stopping this frame...
            if trialnumber.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trialnumber.tStartRefresh + isi + stimDuration-frameTolerance:
                    # keep track of stop time/frame for later
                    trialnumber.tStop = t  # not accounting for scr refresh
                    trialnumber.tStopRefresh = tThisFlipGlobal  # on global time
                    trialnumber.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'trialnumber.stopped')
                    # update status
                    trialnumber.status = FINISHED
                    trialnumber.setAutoDraw(False)
            
            # *crosshair* updates
            
            # if crosshair is starting this frame...
            if crosshair.status == NOT_STARTED and tThisFlip >= stimDuration-frameTolerance:
                # keep track of start time/frame for later
            ...
            if crosshair.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > crosshair.tStartRefresh + isi + stimDuration-frameTolerance:
                    # keep track of stop time/frame for later
                    crosshair.tStop = t  # not accounting for scr refresh
                    crosshair.tStopRefresh = tThisFlipGlobal  # on global time
        ...
        # Check if we've completed 150 trials (a block)
        ...
        block_break = data.Routine(
            name='block_break',
            components=[text_norm_4, key_instruct_4],
        )
        block_break.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # create starting attributes for key_instruct_4
        key_instruct_4.keys = []
        key_instruct_4.rt = []
        _key_instruct_4_allKeys = []
        # Run 'Begin Routine' code from code
        # Skip this routine if it's not time for a break
        ...
        trials.addData('key_instruct_4.keys',key_instruct_4.keys)
        if key_instruct_4.keys != None:  # we had a response
            trials.addData('key_instruct_4.rt', key_instruct_4.rt)
            trials.addData('key_instruct_4.duration', key_instruct_4.duration)
    ...
    # create an object to store info about Routine end
        ...
        name='end',
        components=[text_norm_3, key_instruct_3],
    )
    ...
    # OxySoft event marker - uncomment if using OxySoft
    # oxysoft.WriteEvent("E", "Experiment ended in PsychoPy")
    # Run 'End Experiment' code from code_4
    # Close serial port for TMSI APEX EEG
    ser.close()
